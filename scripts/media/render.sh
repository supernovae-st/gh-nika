#!/usr/bin/env bash
# render.sh — render gh-check.tape against the real engine in a staged demo dir.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
NAME="${1:-gh-check}"
TAPE="$ROOT/scripts/media/$NAME.tape"
[ -f "$TAPE" ] || { echo "no tape at $TAPE" >&2; exit 1; }
command -v vhs >/dev/null || { echo "vhs not installed (brew install vhs)" >&2; exit 1; }
command -v nika >/dev/null || { echo "nika not on PATH" >&2; exit 1; }

SPEC="${NIKA_SPEC_DIR:-$ROOT/../nika-spec}"
SRC="$SPEC/examples/meeting-actions.nika"
[ -f "$SRC" ] || { echo "need $SRC (set NIKA_SPEC_DIR)" >&2; exit 1; }

rm -rf /tmp/ghdemo
mkdir -p /tmp/ghdemo
cp "$SRC" /tmp/ghdemo/meeting-actions.nika
nika check /tmp/ghdemo/meeting-actions.nika >/dev/null || {
  echo "the staged fixture must check clean before it is shown" >&2
  exit 1
}

WORK="$(mktemp -d)"
trap 'rm -rf "$WORK" /tmp/ghdemo' EXIT
cp "$TAPE" "$WORK/$NAME.tape"
(cd "$WORK" && vhs "$NAME.tape")

mkdir -p "$ROOT/media"
OUT="$ROOT/media/$NAME.gif"
if command -v gifsicle >/dev/null; then
  gifsicle -O3 --lossy=40 "$WORK/$NAME.gif" -o "$OUT"
else
  cp "$WORK/$NAME.gif" "$OUT"
fi
SIZE_MB=$(du -m "$OUT" | cut -f1)
[ "$SIZE_MB" -le 8 ] || { echo "✖ $OUT is ${SIZE_MB}MB (budget 8MB)" >&2; exit 1; }
echo "→ $OUT (${SIZE_MB}MB · budget 8MB)"
